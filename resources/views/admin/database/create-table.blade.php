<x-app-layout>
    <x-slot name="header">
        <h1 class="text-2xl font-semibold tracking-tight text-[#0f172a]">New Table</h1>
    </x-slot>
            ...
            <div class="bg-white border border-[#e2e8f0] rounded-xl shadow-sm p-6">
                <form method="POST" action="{{ route('admin.database.store-table') }}"
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button type="button" @click="columns.push({name: '', type: 'string', nullable: false, default: ''})"
                                class="text-sm font-medium text-[#4f46e5] hover:text-[#4338ca]">+ Add column</button>
                        </div>
                                ...
                                <div class="grid grid-cols-12 gap-2 items-start bg-gray-50 rounded-md p-3">
                                    ...
                                    <div class="col-span-3">
                                        <select :name="`columns[${index}][type]`" x-model="column.type"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#4f46e5] focus:ring-[#4f46e5] text-sm">
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        ...
                        <button type="submit" class="px-5 py-2.5 rounded-md bg-[#4f46e5] text-white text-sm font-medium hover:bg-[#4338ca]">Create Table</button>
                    </div>
                </form>
            </div>
        </div>
