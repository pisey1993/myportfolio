    ...
    <article>
        <div class="relative overflow-hidden">
            ...
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-20 pb-8">
                <a href="{{ route('blog.index') }}" class="text-sm text-gray-500 hover:text-white">&larr; All posts</a>

                <p class="mt-4 text-xs text-gray-500">{{ $post->published_at->format('F j, Y') }}</p>
                <h1 class="mt-2 text-3xl sm:text-4xl font-extrabold text-white">{{ $post->title }}</h1>
            </div>
        </div>

        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="aspect-[16/9] rounded-2xl overflow-hidden border border-white/10">
                @if ($post->cover_image)
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-24">
            <div class="prose prose-invert max-w-none text-gray-300 whitespace-pre-line">{{ $post->body }}</div>
        </div>
    </article>
