...
export default function SocialLinks({ links, className = "" }: { links: SocialLinksData; className?: string }) {
  const items = [
    links.githubUrl && {
      href: links.githubUrl,
      label: "GitHub",
      icon: "M12 .5C5.65.5.5 5.65.5 12c0 5.09 3.29 9.4 7.86 10.93.57.1.79-.25.79-.55 0-.27-.01-1.16-.02-2.1-3.2.7-3.88-1.36-3.88-1.36-.52-1.34-1.28-1.69-1.28-1.69-1.04-.72.08-.7.08-.7 1.15.08 1.76 1.19 1.76 1.19 1.03 1.76 2.7 1.25 3.36.96.1-.75.4-1.25.73-1.54-2.55-.29-5.24-1.28-5.24-5.7 0-1.26.45-2.29 1.19-3.09-.12-.29-.52-1.47.11-3.06 0 0 .97-.31 3.18 1.18a11 11 0 0 1 5.79 0c2.2-1.49 3.17-1.18 3.17-1.18.64 1.59.24 2.77.12 3.06.74.8 1.19 1.83 1.19 3.09 0 4.43-2.7 5.4-5.27 5.69.42.36.78 1.07.78 2.16 0 1.56-.01 2.82-.01 3.2 0 .31.21.66.8.55A10.52 10.52 0 0 0 23.5 12c0-6.35-5.15-11.5-11.5-11.5Z",
    },
    links.linkedinUrl && {
    ...
    links.twitterUrl && {
      ...
      label: "Twitter / X",
      icon: "M18.9 2H22l-7.6 8.7L23.3 22h-7.2l-5.6-7.3L4 22H1l8.1-9.3L1 2h7.4l5.1 6.7L18.9 2Zm-1.3 18h1.9L7 4h-2l12.6 16Z",
    },
    links.telegramUrl && {
  ...
  return (
    <div className={`flex items-center gap-4 text-slate-500 dark:text-slate-500 ${className}`}>
      {items.map((item) => (
        <a key={item.label} href={item.href} target="_blank" rel="noopener" aria-label={item.label} className="hover:text-slate-900 dark:hover:text-white transition">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d={item.icon} /></svg>
        </a>
      ))}
      {email && (
        <a href={email.href} aria-label="Email" className="hover:text-slate-900 dark:hover:text-white transition">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
            ...
            <path strokeLinecap="round" strokeLinejoin="round" d="m3.5 6 8.5 6 8.5-6" />
          </svg>
        </a>
      )}
    </div>
